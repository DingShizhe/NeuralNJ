...
def RL_finetuning(cfgs, MSA_file, c_best_tree_file=None, raw_tree_file=None):
    ...
    return the_best_tree, the_best_score, raw_tree_score, c_best_tree_score, time_when_best_score_max, times_when_condition_met, first_time_eq_c_score, relative_rf_distance, the_best_score_20s, the_best_score_50s
...
def RL_Search(cfgs, MSA_file, c_best_tree_file=None, raw_tree_file=None):
    ...
    trajectory_num = 1 #cfgs.ENV.TRAJECTORY_NUM
    # val_path = "/home/dingshizhe/PGPI/data_gen/CSolver/fixed_len_data_n_30_100_test"
    ...
    return the_best_tree, the_best_score, raw_tree_score, c_best_tree_score, time_when_best_score_max, times_when_condition_met, first_time_eq_c_score, relative_rf_distance



def RL_argmax(instance_dir, cfgs, write_dir, write_file_name):
    env = PhyInferEnv(cfgs, device)
    dataset_val = PhyDataset_infer(instance_dir,device=device, load_Csolver=True, load_raw_tree=True)
    batch_size = cfgs.env.batch_size
    data_loader_val = torch.utils.data.DataLoader(dataset_val, batch_size=cfgs.env.batch_size, collate_fn=custom_infer_collate_fn, num_workers=0)
    ...
    start_time = time.time()
    import pandas as pd
    results_df = pd.DataFrame(columns=["File", "Time(avg)", "score", "C Best Tree Score", "RF distance between the best tree and the C best tree", "Raw Tree Score", "RF distance between the best tree and the raw tree", "RF distance between the C best tree and the raw tree", "Taxa number", "Seq Length", "Tree"])
    for batch in data_loader_val:
        file_list = batch["file_paths"]
        c_best_tree_str_list = batch["tree_strs"]
        c_best_tree_score_list = batch["C_logllops"]
        raw_tree_str_list = batch["raw_tree_strs"]
        raw_tree_score_list = batch["raw_logllops"]
        taxa_num_list = batch["taxa_nums"]
        seq_len_list = batch["seq_lens"]
        ...
        # get the RF distance between the best tree and the raw tree
        rf_distance_raw_list = []
        for raw_tree_str, best_tree_str in zip(raw_tree_str_list, best_tree):
            rf_distance, relative_rf_distance = calculate_rf_distance(raw_tree_str, best_tree_str)
            rf_distance_raw_list.append(relative_rf_distance)
        
        # get the RF distance between the C best tree and the raw tree
        rf_distance_c_raw_list = []
        for raw_tree_str, c_best_tree_str in zip(raw_tree_str_list, c_best_tree_str_list):
            rf_distance, relative_rf_distance = calculate_rf_distance(raw_tree_str, c_best_tree_str)
            rf_distance_c_raw_list.append(relative_rf_distance)
        

        # score tensor to list
        scores = [score.detach().cpu().item() for score in scores]
        for file, score, c_best_tree_score, rf_distance, raw_tree_score, rf_distance_raw, rf_distance_c_raw, tree, taxa_num, seq_len in zip(file_list, scores, c_best_tree_score_list, rf_distance_list, raw_tree_score_list, rf_distance_raw_list, rf_distance_c_raw_list, best_tree, taxa_num_list, seq_len_list):
            new_row_df = pd.DataFrame({
                ...
                "RF distance between the best tree and the raw tree": [rf_distance_raw],
                "RF distance between the C best tree and the raw tree": [rf_distance_c_raw],
                "Taxa number": [taxa_num],
                "Seq Length": [seq_len],
                "Tree": [tree]
            })
            results_df = pd.concat([results_df, new_row_df], ignore_index=True)
            print(f"File: {file}, The Best Score: {score}, RF distance between the best tree and the C best tree: {rf_distance}, RF distance between the best tree and the raw tree: {rf_distance_raw}, RF distance between the C best tree and the raw tree: {rf_distance_c_raw}")
...
def Search_inference(test_file_path, write_dir,write_file_name):
    ...
    results_df = pd.DataFrame(columns=["File", "Time When Best Score Max", "Times When Condition Met",
                                    "The Best Score", "C Best Tree Score", "RF distance", "Raw Tree Score", "First Time Eq C Score", "The Best Tree"])
    ...
    for file in files:
        ...
        if all(os.path.exists(f) for f in [MSA_file, c_best_tree_file, raw_tree_file]):
            ...
            the_best_tree, the_best_score, raw_tree_score, c_best_tree_score, time_when_best_score_max, times_when_condition_met, first_time_eq_c_score, rf_distance = RL_Search(cfgs, MSA_file, c_best_tree_file, raw_tree_file)
            ...
            results_df = pd.concat([results_df, new_row_df], ignore_index=True)
            print(f"File: {file}, The Best Score: {the_best_score}, Time When Best Score Max: {time_when_best_score_max}, Times When Condition Met: {times_when_condition_met}, RF distance: {rf_distance}, C Best Tree Score: {c_best_tree_score}, Raw Tree Score: {raw_tree_score}, First Time Eq C Score: {first_time_eq_c_score}")
...
def finetune_inference(test_file_path, write_fir, write_file_name):
    ...
    # Initialize a DataFrame to hold all the results
    results_df = pd.DataFrame(columns=["File", "Time When Best Score Max", "Times When Condition Met",
                                    "The Best Score", "C Best Tree Score", "RF distance", "Raw Tree Score", "First Time Eq C Score", "The Best Tree", "20s score", "50s score"])
    for file in files:
        MSA_file = os.path.join(test_file_path, file)
        c_best_tree_file = os.path.join(cfgs.instance_path, file[:-4] + ".tre")
        raw_tree_file = os.path.join(cfgs.instance_path, file[:-4] + "_raw.tre")
        if all(os.path.exists(f) for f in [MSA_file, c_best_tree_file, raw_tree_file]):
            # Call the placeholder RL_Search function
            the_best_tree, the_best_score, raw_tree_score, c_best_tree_score, time_when_best_score_max, times_when_condition_met, first_time_eq_c_score, rf_distance, the_best_score_20s, the_best_score_50s = RL_finetuning(cfgs, MSA_file, c_best_tree_file, raw_tree_file)
            # Append the results to the DataFrame
            new_row_df = pd.DataFrame({
                "File": [file],
                "The Best Score": [the_best_score],
...
if __name__ == "__main__":
    ...
    config_file_name = "20240503-135822taxa50_lenmixed2000"
    ...
    if root_dir == True:
        test_file_path_list = []
        for len_folder in os.listdir(test_file_dir):
            len_path = os.path.join(test_file_dir, len_folder)
            if os.path.isdir(len_path):
                for taxa_folder in os.listdir(len_path):
                    taxa_path = os.path.join(len_path, taxa_folder)
                    test_file_path_list.append(taxa_path)
        print(test_file_path_list)

        write_dir =  f"/home/dingshizhe/PhyloNet/search/{label}_{config_file_name}_dim{cfgs.model.embed_dim}_patch{cfgs.model.patch_size}/"

        if not os.path.exists(write_dir):
            os.makedirs(write_dir)

        shutil.copy("/home/dingshizhe/PhyloNet/search/get_plot_seqlen_meanLHDiff.py", write_dir)
        
        for test_file_path in test_file_path_list:
            test_file_name = test_file_path.split('/')[-3:]
            write_file_name = f"{label}_bs{cfgs.env.batch_size}_{test_file_name[0]}_{test_file_name[1]}_{test_file_name[2]}.csv"
        ...
        custom_tag = "20240503-135822taxa50_lenmixed2000_checkpoint100"

        test_file_path = test_file_dir
        test_file_name = test_file_path.split('/')[-3:]
        write_dir =  f"/home/dingshizhe/PhyloNet/search/{label}_{custom_tag}_dim{cfgs.model.embed_dim}_patch{cfgs.model.patch_size}/"
        write_file_name = f"{label}_bs{cfgs.env.batch_size}_{test_file_name[0]}_{test_file_name[1]}_{test_file_name[2]}_time{STOP_TIME}.csv"
